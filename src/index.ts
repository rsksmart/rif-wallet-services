...
app.get('/tokens', async (request: Request, response: Response, next: NextFunction) => {
    console.log(request.path)
    response.status(200).json(await api.getTokens());
    
})

app.get('/address/:address/tokens', async (request: Request, response: Response, next: NextFunction) => {
    console.log(request.path)
...
app.get('/address/:address/events', async (request: Request, response: Response, next: NextFunction) => {
    console.log(request.path)
    const address = request.params.address
    if(!address) return response.status(404)
    if(isValidAddress(address)) {
...
app.get('/address/:address/transactions', async (request: Request, response: Response, next: NextFunction) => {
    console.log(request.path)
    const address = request.params.address
    if(!address) return response.status(404)
    if(isValidAddress(address)) {
